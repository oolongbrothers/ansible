...
options:
  name:
    ...
    - When supplying an `http(s)` URL with `absent`, the module will try to match the
      installed flatpak based on the name of the flatpakref to remove it.
    - However, there is no guarantee that the names of flatpakref file and the reverse DNS name
      of the installed flatpak do match. This depends on the flatpak repository remote you are
      using.
    required: true
  executable:
    description:
